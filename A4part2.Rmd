---
title: "Assignment 4, part2"
author: "Lilja Bjorg, Jón Vilberg, Daníeln  Bergmann"
...
Xvar2013 = data.frame(price = dat$Price[169:(nSamp)], 
...
err = c(mdl2013$residuals, rep(0,(length(Pt)-length(mdl2013$residuals))))
err = as.numeric(err)
XX = cbind(P0,Wt,Lt,Pt168,Pt24,Pt1,err)
```
Fall sem gerir recursive parametric estimate
```{r}
recurse_est = function(XX,Y,lmbd){
  ...
  for(tt in 2:n){  
    ...
    xt <- XX[tt,]
    if(is.na(Y[tt]){ #for future predictions
      Y[tt]=Y[tt-1]
    } 
    #Update Rt
    Rt <- xt%*%t(xt) + lmbd*Rt
    ...
    if(class(theta.try) != "try-error"){
      ...
      XX[tt+1,7] = (yhat[tt+1]- Pt[tt+1])  # updating residual in XX matrix
    }
  }
  yhat[yhat > 600]= 600
  return(yhat)  
...
```
Fall sem reiknar root mean square error
```{r}
...
Looks as though no forgetting factor gives least RMSE, going for 0.55 because it is plausible that it will improve the model
```{r}
nSamp = 1000
...
P0 = rep(1,(nSampYear-168))
err = c(mdl2014$residuals, rep(0,(length(Pt)-length(mdl2014$residuals))))
err = as.numeric(err)
XX = cbind(P0,Wt,Lt,Pt168,Pt24,Pt1,err)
```
increase XX dimension into future
```{r}
forecast_xx = matrix(rep(XX[6817,],times=13),nrow=13,ncol=7,byrow=T) # i
XX = rbind(XX, forecast_xx)
yhat2014 = ts(recurse_est(XX,Y,0.55),start=c(2014,1),frequency=24*365)
plot(yhat2014[(length(yhat2014)-100):length(yhat2014)] ,
     type='l',col=2,ylab="Spot Price")
lines(yhat2014[(length(yhat2014)-13):length(yhat2014)] ,
     type='l',col=1)
grid()

(prediction = yhat2014[(length(yhat2014)-12):length(yhat2014)])

```
Obviously there is an bug in the code that does not allow for future predited values. Theory is that the Rt matrix is not singular